        ...
        "<Имя элемента без цифрового ID>": [
            {
                ...
                "positions": [
                    {"id": 1391, "name": "Стена надземной части здания толщиной до 200 мм",
                     "group": "до 57"}        // группа работ по высоте (?group= ссылки;
                ]                             //   пустая строка, если работы от высоты не зависят)
            },
            ...
        ],
        ...
...
Часть здания варианта определяется СТРОГО по «Расположению»/«Этажу»
(числовому индикатору: «-N/M» → цоколь, «-N» → подземная, «N» →
надземная; см. _resolve_part) — синхронно с разделением смешанных групп
при подборе работ (ifc_reference_builder._element_part). Уточнение по
«Типу этажа» не выполняется: отметка 1-го этажа (0,000) ошибочно
размечала его как цокольный. Если часть варианта расходится
...
работы сгруппированы по диапазонам высоты здания («до 57», «более 57 до
75 м», «более 75 до 105 м» …) — карточка такой позиции в ЦС открывается по
ссылке с параметром ?group=<имя группы> (например,
.../position/464?group=до+57). Имя группы приходит в поле group элемента
workGroups ответа API; позициям, у которых ни у одной группы работ нет
диапазона высоты (работы от высоты не зависят, например «Фундаментная
плата»), параметр не нужен — ссылка остаётся без него. Группа выбирается по
фактической высоте здания: после фильтрации ответа по высоте
(api_works_lookup._filter_response_by_height) у позиции остаются только
подходящие по высоте группы работ — берётся имя первой из них с диапазоном
(см. _position_height_group). Если высота здания не определена, параметр
не добавляется (выбрать группу нельзя). Группа сохраняется в поле group
позиции в position_links.json, фронтенд подставляет её в ссылку.
...
def _resolve_part(
    ...
    """Часть здания элемента — строго по «Расположению»/«Этажу».

    База — результат _extract_part («Расположение» → «Этаж» → «Тип этажа»).
    Уточнение по «Типу этажа» не выполняется: модели размечают 1-й этаж
    (отметка 0,000) как «Цокольный», хотя числовой индикатор «Этажа»
    («1_этаж_основной») относит его к надземной части — та же строгая
    логика, что в ifc_reference_builder._element_part (разделение
    смешанных групп по частям здания при подборе работ).
    """
    return _extract_part(characteristics, additional) or "Надземная"
...
def _override_location(payload: Dict[str, Any], part: str) -> bool:
    ...
    Часть здания варианта определяется по числовому индикатору «Этажа»
    (см. _resolve_part), а характеристика «Расположение» элемента могла
    быть построена иначе. Без подмены API вернёт позиции не той части
    здания — ссылка укажет на карточку другой позиции (например,
    «Стена надземной части…» вместо «Стена цокольной части…»).
...
def _position_height_group(position: Dict[str, Any]) -> str:
    """Имя группы работ по высоте здания у позиции (параметр ?group= ссылки).

    Работы позиции ЦС сгруппированы по диапазонам высоты здания («до 57»,
    «более 57 до 75 м», «более 75 до 105 м» …) — имя группы приходит в поле
    group элемента workGroups, диапазон — в поле range. Если ни у одной
    группы работ позиции нет диапазона высоты (работы от высоты не зависят,
    например «Фундаментная плита»), параметр ссылке не нужен — пустая строка.

    Вызывается для позиций, уже отфильтрованных по высоте здания
    (api_works_lookup._filter_response_by_height): у позиции остались только
    подходящие по высоте группы, поэтому имя первой группы с диапазоном —
    группа именно для этой высоты.
    """
    for group in position.get("workGroups") or []:
        if not isinstance(group, dict):
            continue
        if not group.get("range"):
            # Группа без диапазона высоты (например, «Уход за бетоном») —
            # применяется при любой высоте, имя не используется в ссылке
            continue
...
def _extract_positions(
    ...
    Для каждой позиции дополнительно определяется группа работ по высоте
    здания (поле group позиции, см. _position_height_group) — параметр
    ?group= ссылки на карточку позиции в ЦС. Группа определяется только
    при известной высоте здания: ответ уже отфильтрован по высоте, и у
    позиции остались только подходящие группы работ. Если высота не
    определена, у позиций не осталось однозначной группы — параметр
    не добавляется (group = "").
    ...
    raw = [
        {
            ...
            # Группа работ по высоте (параметр ?group= ссылки) — только при
            # известной высоте: ответ отфильтрован, у позиции остались только
            # подходящие группы; без высоты группу выбрать нельзя
            "group": (
                _position_height_group(item) if building_height is not None else ""
            ),
        }
        for item in data
    ]
